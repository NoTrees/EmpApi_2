...
module API::V1
	class ListingWorkTimesTest < ActionDispatch::IntegrationTest
		test 'return list of work times' do
			get '/work_times', {}, { 'Authorization' => token_header(@user.auth_token), 'Accept' => Mime::JSON }
		...
		test 'return list of work times in XML' do
			get '/work_times', {}, { 'Authorization' => token_header(@user.auth_token), 'Accept' => Mime::XML }
			assert_equal 200, response.status
			assert_equal Mime::XML, response.content_type
		end

		test 'return work times filtered by id' do
			get '/work_times?employee_id=12345', {}, { 'Authorization' => token_header(@user.auth_token), 'Accept' => Mime::JSON }
			assert_equal 200, response.status
		...
		test 'return work times filtered by work date' do
			get '/work_times?work_date=2015-05-14', {}, { 'Authorization' => token_header(@user.auth_token), 'Accept' => Mime::JSON }
			assert_equal 200, response.status
